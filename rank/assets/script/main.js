...
cc.Class({
    ...
    additem: function()
    {
        ...
        if(this.content.childrenCount<4 && i<this.friendRank.length)
        {
            ...
            var head = cc.find("head",item);
            item.tid = id;
            num.string = (i+1);
            name.string = this.getLabelStr(data.nickname,8);
            score.string = rankdata.wxgame.score+"关";
    ...
    sortFriendRank: function()
    {
        if(this.friendRank)
        {
            ...
            while(true)
            {
                ...
                {
                    if(this.friendRank[i].KVDataList.length<=0)
                    {
                        this.friendRank.splice(i,1);
                        b = true;
                        break;
                    }
                }
